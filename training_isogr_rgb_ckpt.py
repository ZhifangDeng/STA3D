...
training_datalist = '/home/dzf/STA3D/trte_splits_aug/IsoGD_Image/train_rgb_list.txt'
testing_datalist = '/home/dzf/STA3D/trte_splits_aug/IsoGD_Image/valid_rgb_list.txt'
model_prefix='./model_depth_train'
...
l2_cost = weight_decay * tf.add_n([tf.nn.l2_loss(var) for var in tf.trainable_variables()])
...
lr2 = tf.train.exponential_decay(learning_rate*10,
                                ...
                                decay_rate,
                                staircase=True)
var_list1 = networks.all_params[:662] 
var_list2 = networks.all_params[662:]
...
if start_step>0:
  load_params = tl.files.load_ckpt(sess = sess, mode_name='%s_sta3d_model_epoch_%d.ckpt'%(dataset_name, start_epoch))
  #tl.files.assign_params(sess, load_params, networks)
  print 'load param from %s_sta3d_model_epoch_%d.ckpt'%( dataset_name, start_epoch)
...
X_data_a  = np.empty((batch_size*queue_num, seq_len, 224, 224, 3),float)      
...
for epoch in range(start_epoch, n_epoch):
  ...
  tl.files.save_ckpt(sess = sess, 
                    mode_name='%s_sta3d_model_epoch_%d.ckpt'%(dataset_name, epoch+1), 
                    printable = True)
  print("Model saved in file: %s_sta3d_model_epoch_%d.ckpt" %(dataset_name, epoch+1))
  log.writelines(['Model saved in file: %s_sta3d_model_epoch_%d.ckpt'%(dataset_name, epoch+1), 
                  '\n'])
  log.flush()
